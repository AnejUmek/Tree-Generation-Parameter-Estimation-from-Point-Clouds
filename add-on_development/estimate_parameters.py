    ...
    if obj:
        if obj.type == "MESH":
            if len(obj.data.vertices) > 0 and len(obj.data.edges) == 0 and len(obj.data.polygons) == 0:
                ...
                mean_path = Path(__file__).parent.parent.joinpath("mean.npy")
                std_path = Path(__file__).parent.parent.joinpath("std.npy")
...
def calculate_parameters(point_cloud):
    ...
    checkpoint_path = Path(__file__).parent.parent.joinpath("model.pth")
